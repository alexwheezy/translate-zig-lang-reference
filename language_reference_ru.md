...
результирующий исполняемый файл будет _непригоден для копирования на другой компьютер_. Чтобы скопировать исполняемый файл
на другой компьютер, компилятору необходимо узнать о целевых требованиях с помощью параметра `-target`.

Стандартная библиотека Zig (`@import("std")`) имеет кроссплатформенные абстракции, что делает один и тот же исходный код
пригодным для многих целей. Некоторые коды более переносимы чем другие. В целом, Zig-код чрезвычайно переносим по
...
Этот пример является исключением из правила, указанного в разделе "Avoid Redundancy in Names". Значение типа было
сведено к его сути: это значение в формате `json`. Название не может быть более конкретным не будучи неправильным.
...
акронимы, состоящие всего из 2 букв подчиняются этим соглашениям.
...
Дополнительные примеры приведены в стандартной библиотеке Zig.
...
байтовое значение завершает каждую строку исходного кода Zig, за исключением последней строки файла. Рекомендуется,
...
табуляции не рекомендуется. Смотрите грамматику.
...
`/\n/`, или расширенный поиск, такой как `/\r\n?|[\n\u0085\u2028\u2029]/`, и в том, и в другом случае окончания строк
будут определены правильно. Для другого примера, при определении пробела перед первым маркером в строке инструмент может
либо использовать простой поиск, такой как `/[\t]/`, или расширенный поиск, такой как `/\s/` и в любом случае пробелы
будут правильно идентифицированы.
...
|`if`|Выражение `if` может проверять логические выражения, опциональные значения или объединения с ошибками. Для опциональных значений или объединений с ошибками выражение `if` может захватывать развернутое значение. Смотрите также [if](#if).|
